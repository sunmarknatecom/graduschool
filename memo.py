...
for elem in idx_list:
    raw_ct_image, raw_lb_image, ct_iamge, nm_image, lb_image = get_images(elem)
    print(elem, np.shape(raw_ct_image), np.shape(raw_lb_image), np.shape(ct_iamge), np.shape(nm_image), np.shape(lb_image))
